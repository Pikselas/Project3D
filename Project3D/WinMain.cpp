...
int _stdcall WinMain(HINSTANCE hinstance, HINSTANCE hPrev, LPSTR lpcmd, int cmdshow)
{
    ...
    while (true)
    {
        ...
        D3D11_BUFFER_DESC indB = { 0 };
        indB.ByteWidth = sizeof(indx);
        indB.Usage = D3D11_USAGE_DEFAULT;
        indB.BindFlags = D3D11_BIND_INDEX_BUFFER;
        indB.CPUAccessFlags = 0u;
        indB.MiscFlags = 0u;
        indB.StructureByteStride = sizeof(unsigned short);
        D3D11_SUBRESOURCE_DATA sInd = { 0 };
        sInd.pSysMem = indx;
        graphics->CreateBuffer(&indB, &sInd, &IBuffer);
        ...
        context->DrawIndexed((UINT)std::size(indx), 0u, 0u);
    }
    return 0;
}